...
const FriendRequestsDropdown = () => {
  ...
  const { data: addFriendRequests, error } = useQuery({
    queryKey: "friendRequests",
    queryFn: async () => {
      try {
        const response = await api.get(
          "http://localhost:8080/employee/get/friend-requests"
        );

        // Kiểm tra mã trạng thái 204 No Content
        if (response.status === 204) {
          return []; // Trả về một mảng trống khi không có dữ liệu
        }

        return response.data;
      } catch (error) {
        console.error("Error fetching friend requests:", error);
        throw error; // Đẩy lỗi ra ngoài để có thể xử lý
      }
    },
  });
  ...
  const handleAccept = async (id) => {
    try {
      await api.post(
        `http://localhost:8080/employee/accept-friend/request/${id}`,
        null
      );
  ...
  const handleDeleteFriendRequest = async (id) => {
    try {
      await api.delete(
        `http://localhost:8080/employee/delete/friend-request/${id}`,
        null
      );
      setFriendRequests((prevRequests) =>
        prevRequests.filter((request) => request.id !== id)
